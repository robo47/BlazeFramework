...
class ExpressionOperation extends Object{
	...
	public function __construct($expressionString){
		...
		$operations = $tokenizer->tokenize($expressionString);
                ...
                if(count($operations) == 1){
                    ...
                    foreach($this->operatorLevels[0] as $functionName){
                            $pos = stripos($operations[0]['leftExpression'],$functionName);
                            ...
                            if($pos !== false){
                                    ...
                                    if($pos != 0)
                                            $string = substr($operations[0]['leftExpression'], 0, $pos);
                                    $string .= substr($operations[0]['leftExpression'], $pos + strlen($functionName), strlen($operations[0]['leftExpression']));
                                    // Make a new function object
                                    $this->rootOperation = $this->getFunctionObject($functionName, $string);
	...
	public function getOperationObject($operator, $left, $right){
		switch($operator){
			case '*':
	...
	public function getFunctionObject($functionName, $content){
		switch($functionName){
			case 'empty':
